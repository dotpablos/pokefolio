...
export const InfoRegistry: IdContentMap = {
  "about-me": {
  ...
  pokefolio: {
    ...
    description:
      "What you're seeing right now! A Pokemon theme developer portfolio built with React, Next.js and RPGMaker. This concept was inspired by a previous boss of mine who had a Mario Galexy theme portfolio where the user could fly to different galexies to learn about his previous projects. I decided to take this concept and apply it to my own portfolio, but with a Pokemon theme instead.",
    description2:
      "This portfolio is comprised of two major pieces: the HTML content that displays the project information, and the RPGMaker game map that allows the user to navigate to different sections of the portfolio. The HTML content is built with React, Next.js and TailwindCSS, and the RPGMaker map is built with RPGMaker MV. In order to bridge the two, I created a custom buffer reader that allows the webpage to synchronize the game's state in order to display the correct content for the user when entering the different houses in game. While not a full-fledged game, it did require a lot of creativity and problem solving to get the two to work together, especially implementing the gameboy styled controls when the page is being viewed on a mobile device (Try it out on your phone!).",
    workDetails: {
      detail1: {
        type: "Start Date",
        detail: "January 2026",
      ...
      detail2: {
        type: "Technologies",
        detail: "React, Next.js, Node.js, TailwindCSS, RPGMaker MV",
      },
    },
  },
  credits: {
    ...
    workDetails: {
      ...
      detail2: {
        type: "Pokemon Essentials (RPGMaker Plugin)",
...
export type CardID = keyof typeof InfoRegistry;
